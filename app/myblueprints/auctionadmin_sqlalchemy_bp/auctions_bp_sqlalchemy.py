...
from myblueprints.auctionadmin_sqlalchemy_bp.auction_repository import AuctionRepository
...
def get_all_auctions():
    ...
    return render_template('auctionviews/auctions_list.html', auctions=auctions_list)

@auctions_bp_sqlalchemy.route('/<int:auction_id>', methods=['GET'])
def get_auction_by_id(auction_id):
    auction = auctions_repo.find_by_id(auction_id)
    if auction:
        return render_template('auctionviews/auction_detail.html', auction=auction)
    else:
